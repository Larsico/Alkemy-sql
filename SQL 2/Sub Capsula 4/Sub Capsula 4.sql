...
SELECT COUNT(*) FROM(SELECT p.nombre, apellido, COUNT(c.nombre) as Cursos, c.turno FROM profesores as p INNER JOIN curso as c ON (c.profesor_id = p.id) WHERE turno = 'Noche' GROUP BY p.nombre HAVING Cursos > 1) as Cantidad
